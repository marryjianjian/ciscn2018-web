...
from sshop.settings import key, flag
    ...
    def post(self):
        ...
        if username and password:
            ...
            if user.check(password):
                ...
                print 'username: ', username, type(username), username.encode('hex')
                # username = base64.b64encode(username.decode())
                # print 'username: ', username, type(username), username.encode('hex')
                print 'user_cookie : ', user_cookie, type(user_cookie)
                self.set_cookie('username', username.encode('hex'))
                self.set_cookie('user_cookie', user_cookie)
                self.set_cookie('secretkey_length', str(len(key)))
                self.redirect('/user')
            else:
                return self.render('login.html', danger=1, ques=self.application.question, uuid=self.application.uuid)
...
class RegisterHandler(BaseHandler):
    ...
    def post(self, *args, **kwargs):
        ...
        if 'vip' in username.lower():
            return self.render('register.html', danger=1, ques=self.application.question, uuid=self.application.uuid, message='You can\'t be vip')
...
class changePasswordHandler(BaseHandler):
    ...
    def post(self, *args, **kwargs):
        ...
        user = self.orm.query(User).filter(User.username == self.current_user).first()
        if password == password_confirm:
...
class UserInfoHandler(BaseHandler):
    ...
    def get(self, *args, **kwargs):
        ...
        user = self.orm.query(User).filter(User.username == self.current_user).first()
        if user.is_vip():
            return self.render('user.html', user=user, success=1, message=flag)
        else:
...
class UserLogoutHandler(BaseHandler):
    ...
    def get(self, *args, **kwargs):
        ...
        self.clear_cookie('secretkey_length')
        self.redirect('/login')
